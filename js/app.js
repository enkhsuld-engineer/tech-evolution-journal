...
const PROJECTS_SOON = true; // set false when you want to show real milestones/proof
/* =========================
...
async function loadProjects(){
  // sample projects are in i18n (strings) + here (numbers/structure)
  PROJECTS = [
    {
      id:'inv-stack',
      pinned:true,
      title_key:'projects.samples.inv_stack.title',
      meta_key:'projects.samples.inv_stack.meta',
      desc_key:'projects.samples.inv_stack.desc',
      milestones:[
        { name_key:'projects.samples.inv_stack.ms1', pct:0 },
        { name_key:'projects.samples.inv_stack.ms2', pct:40 },
        { name_key:'projects.samples.inv_stack.ms3', pct:20 },
      ],
      proof_keys:[
        'projects.samples.inv_stack.p1',
        'projects.samples.inv_stack.p2',
        'projects.samples.inv_stack.p3',
      ]
    },
    {
      id:'dual-mcu',
      pinned:true,
      title_key:'projects.samples.dual_mcu.title',
      meta_key:'projects.samples.dual_mcu.meta',
      desc_key:'projects.samples.dual_mcu.desc',
      milestones:[
        { name_key:'projects.samples.dual_mcu.ms1', pct:0 },
        { name_key:'projects.samples.dual_mcu.ms2', pct:15 },
        { name_key:'projects.samples.dual_mcu.ms3', pct:0 },
      ],
      proof_keys:[
        'projects.samples.dual_mcu.p1',
        'projects.samples.dual_mcu.p2',
        'projects.samples.dual_mcu.p3',
      ]
    },
    {
      id:'buck-current',
      pinned:false,
      title_key:'projects.samples.buck_current.title',
      meta_key:'projects.samples.buck_current.meta',
      desc_key:'projects.samples.buck_current.desc',
      milestones:[
        { name_key:'projects.samples.buck_current.ms1', pct:0 },
        { name_key:'projects.samples.buck_current.ms2', pct:50 },
        { name_key:'projects.samples.buck_current.ms3', pct:10 },
      ],
      proof_keys:[
        'projects.samples.buck_current.p1',
        'projects.samples.buck_current.p2',
        'projects.samples.buck_current.p3',
      ]
    },
  ];
}
...
function renderPinnedProjects(){
  ...
  el.innerHTML = pins.map(p=>`
    <div class="pinItem" data-proj="${escapeAttr(p.id)}" tabindex="0">
      ...
      <div class="pinMeta">${p.milestones?.[0]?.pct ?? 0}%</div>
    </div>
  `).join('');
...
function renderProjects(activeId){
  ...
  ${PROJECTS_SOON ? 'SOON' : `${p.milestones?.[0]?.pct ?? 0}%`}
</span>
            </button>
          `).join('')}
        </div>
  ...
  el.querySelectorAll('[data-proj]').forEach(btn=>{
    btn.onclick = ()=> renderProjects(btn.getAttribute('data-proj'));
  });
}
...
function renderProjectCard(p){
  if(PROJECTS_SOON){
    return `
      <div class="projCard">
        ...
        <div class="soonBox" style="margin-top:12px">
          <div style="font-weight:950;letter-spacing:.08em">SOON</div>
          <div style="margin-top:6px;color:var(--muted);line-height:1.6">
            Projects are not published yet. This page will be updated as builds reach measurable validation (simulation → firmware → bench results).
          </div>
        </div>
  ...
  const ms = (p.milestones||[]).map(m=>{
    ...
    return `
      <div class="ms">
        <div style="flex:1">
          <div class="msName">${escapeHtml(t(m.name_key))}</div>
          <div class="bar" style="margin-top:8px"><div style="width:${pct}%"></div></div>
        </div>
        <div class="msPct">${pct}%</div>
      </div>
    `;
  }).join('');

  const proof = (p.proof_keys||[]).map(k=>`<li>${escapeHtml(t(k))}</li>`).join('');
  ...
  return `
    <div class="projCard">
      ...
      <div style="font-weight:950;margin-top:12px">${escapeHtml(t('projects.milestones'))}</div>
      <div class="milestones">${ms}</div>

      <div style="font-weight:950;margin-top:14px">${escapeHtml(t('projects.proof'))}</div>
      <ul style="margin:8px 0 0 18px;color:var(--muted)">${proof}</ul>
  ...
  el.innerHTML = `
    ...
    <div class="tools-wrap">
      <aside class="tools-sidebar">
        ...
        <span>CAS</span>
        <span class="tool-status">SymPy</span>
      </button>
