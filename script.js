...
const I18N = {
  en: {
    siteTitle: 'Tech Study Journal | Altanbaatar Enkhsuld',
    navHome: 'Home',
    navAbout: 'About',
    navTags: 'Tags',
  ...
  ja: {
    ...
    mn: {
    siteTitle: "Техникийн Судалгааны Тэмдэглэл | Altanbaatar Enkhsuld",
    navHome: "Нүүр",
    navAbout: "Тухай",
    navTags: "Тагууд",
    ...
    heroLead: "Бичлэг бүр нь мини-нийтлэл: юу туршсан, яаж туршсан, ямар үр дүн гарсан, юу сурсан тухай.",
...
async function renderArticle(id){
  ...
  // choose content path per language (if you add content_ja later)
  const contentUrl = (getLang()==='ja' && p.content_ja) ? p.content_ja : p.content;
  ...
  if((p.type||'').includes('md')){
    ...
    const summary = (getLang()==='ja' && p.summary_ja) ? p.summary_ja : (p.summary||'');
  ...
  const sep='<span class="dot">&middot;</span>';
  const meta=metaBits.join(sep);
  const heroBlock=p.hero?`<img src="${p.hero}" alt="" style="width:100%;height:auto;border-radius:12px;border:1px solid var(--border);margin:8px 0 14px">`:'';
  const pdfBlock=p.pdfUrl?`
    <div class="card" style="padding:0;margin-top:18px;overflow:hidden">
      <div style="padding:16px;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center">
        ...
        <a class="btn" href="${p.pdfUrl}" target="_blank" rel="noopener">${t('openPdf')}</a>
      </div>
      <div class="pdfwrap"><object data="${p.pdfUrl}" type="application/pdf"></object></div>
    </div>`:'';
