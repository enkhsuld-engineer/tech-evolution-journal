...
const I18N = {
  en: {
    ...
    toolsLead: 'Small calculators and visualizers (Bode, Laplace, Euler, etc.).',
    toolBode: 'Bode Diagram',
    toolSoon: 'Soon',
    plot: 'Plot',
    reset: 'Reset',
  },
  ja: {
    ...
    navTools: 'Tools',
    toolsTitle: 'Engineering Tools',
    toolsLead: 'Bode / Laplace / Euler などの小ツール。',
    toolBode: 'Bode Diagram',
    toolSoon: 'Soon',
    plot: 'Plot',
    reset: 'Reset',
    ...
    navTools: 'Tools',
    toolsTitle: 'Engineering Tools',
    toolsLead: 'Жижиг тооцоолуур, визуал хэрэгсэл (Bode, Laplace, Euler гэх мэт).',
    toolBode: 'Bode Diagram',
    toolSoon: 'Soon',
    plot: 'Plot',
    reset: 'Reset',
...
function applyStaticTexts(){
  ...
  navLinks.forEach(a=>{
    ...
    if(href==='#/tools') a.textContent = t('navTools');
...
function renderTools(){
  ...
  el.innerHTML = `
    <div class="card" style="padding:20px">
      <h2 style="margin:0 0 6px">${t('toolsTitle')}</h2>
      <p style="margin:0;color:var(--muted)">${t('toolsLead')}</p>
    </div>
    ...
    <div class="tools-wrap">
      <aside class="tools-sidebar">
        <button class="tool-item ${active==='bode'?'active':''}" data-tool="bode">
          <span>${t('toolBode')}</span>
          <span class="tool-status">Ready</span>
        </button>

        <button class="tool-item ${active==='laplace'?'active':''}" data-tool="laplace">
          <span>Laplace</span>
          <span class="tool-status">Ready</span>
        </button>
      ...
      <main class="tools-main">
        ${
          active === 'bode' ? bodeToolHtml() :
          active === 'laplace' ? laplaceToolHtml() :
          active === 'euler' ? eulerToolHtml() :
          comingSoonHtml(active)
        }
      </main>
    </div>
  `;
  ...
  // Wire active tool
  if(active === 'bode'){
    const plotBtn = document.getElementById('bodePlotBtn');
    const resetBtn = document.getElementById('bodeResetBtn');
    if(plotBtn) plotBtn.onclick = ()=> bodePlotFromUi(false);
    if(resetBtn) resetBtn.onclick = ()=> bodeReset();
    bodePlotFromUi(true);
  }

  if(active === 'laplace'){
    const calcBtn = document.getElementById('lapCalcBtn');
    const resetBtn = document.getElementById('lapResetBtn');
    if(calcBtn) calcBtn.onclick = ()=> laplaceRender();
    if(resetBtn) resetBtn.onclick = ()=> laplaceReset();
    laplaceRender();
  }

  if(active === 'euler'){
    const r2p = document.getElementById('euRectToPolarBtn');
    const p2r = document.getElementById('euPolarToRectBtn');
    const ej  = document.getElementById('euEulerBtn');
    const rst = document.getElementById('euResetBtn');

    if(r2p) r2p.onclick = ()=> eulerRectToPolar();
    if(p2r) p2r.onclick = ()=> eulerPolarToRect();
    if(ej)  ej.onclick  = ()=> eulerComputeEj();
    if(rst) rst.onclick = ()=> eulerReset();
...
function comingSoonHtml(name){
  return `
    ...
    <div class="tool-sub">Not implemented yet.</div>
    <div class="tool-help">
      Add this tool later by implementing a renderer like <b>bodeToolHtml()</b> and wiring it inside <b>renderTools()</b>.
    </div>
  `;
}

function bodeToolHtml(){
  return `
    <div class="tool-title">${t('toolBode')}</div>
    <div class="tool-sub">Continuous-time Bode plot for G(s) = N(s)/D(s). PSIM-style numerator/denominator coefficients.</div>

    <div class="tool-grid">
      <div class="tool-field">
        <label>Numerator coefficients (highest power → constant)</label>
        <textarea id="bodeNum">1</textarea>
      </div>
      <div class="tool-field">
        <label>Denominator coefficients (highest power → constant)</label>
        <textarea id="bodeDen">0.0002, 0.01</textarea>
      </div>

      <div class="tool-field">
        <label>f_min (Hz)</label>
        <input id="bodeFmin" type="number" value="1" step="0.1"/>
      </div>
      <div class="tool-field">
        <label>f_max (Hz)</label>
        <input id="bodeFmax" type="number" value="20000" step="1"/>
      </div>

      <div class="tool-field">
        <label>Points (log sweep)</label>
        <input id="bodeNpts" type="number" value="800" step="50"/>
      </div>
      <div class="tool-field">
        <label>Delay T_d (s) optional (phase only)</label>
        <input id="bodeDelay" type="number" value="0" step="0.000001"/>
      </div>
    </div>

    <div class="tool-actions">
      <button class="btn" id="bodePlotBtn">${t('plot')}</button>
      <button class="btn ghost" id="bodeResetBtn">${t('reset')}</button>
    </div>

    <div class="tool-help">
      <b>Quick use (PSIM-style):</b><br/>
      1) Enter coefficients from highest power of s down to constant term.<br/>
      2) Missing terms must be entered as 0 (e.g., s² + 400 → [1, 0, 400]).<br/><br/>
      <b>RL plant:</b> G(s)=1/(L·s+R) → Num [1], Den [L, R]<br/>
      <b>PI:</b> (Kp·s+Ki)/s → Num [Kp, Ki], Den [1, 0]
    </div>
...
// Evaluate polynomial with real coefficients at s=j*w
// coeffs: [c0..cn] means c0*s^(n) + ... + cn
function bodePolyEvalJw(coeffs, w){
  let re = 0, im = 0;
  const sre = 0, sim = w; // s=j*w
...
function bodeCDiv(a,b){
  const d = b.re*b.re + b.im*b.im;
  if(d === 0) return {re: Infinity, im: Infinity};
  return {re:(a.re*b.re + a.im*b.im)/d, im:(a.im*b.re - a.re*b.im)/d};
}
function bodeCAbs(a){ return Math.hypot(a.re, a.im); }
function bodeCArgDeg(a){ return Math.atan2(a.im, a.re) * 180 / Math.PI; }

function bodeLogspace(fmin, fmax, n){
  const out = [];
...
function bodePlotFromUi(silent){
  try{
    const num = bodeParseCoeffs(document.getElementById('bodeNum')?.value);
    const den = bodeParseCoeffs(document.getElementById('bodeDen')?.value);
    const fmin = Number(document.getElementById('bodeFmin')?.value);
    const fmax = Number(document.getElementById('bodeFmax')?.value);
    const npts = Math.max(50, Math.floor(Number(document.getElementById('bodeNpts')?.value)));
    const Td = Number(document.getElementById('bodeDelay')?.value);
    ...
    const f = bodeLogspace(fmin, fmax, npts);
    const magDb = [];
    const phDeg = [];

    for(let i=0;i<f.length;i++){
      const w = 2*Math.PI*f[i];
      const N = bodePolyEvalJw(num, w);
      const D = bodePolyEvalJw(den, w);
      const G = bodeCDiv(N, D);

      const mag = bodeCAbs(G);
      const phaseDelayDeg = -(w * Td) * 180 / Math.PI;
      const phase = bodeCArgDeg(G) + phaseDelayDeg;
    ...
    Plotly.newPlot('bodeMag', [{
      x: f, y: magDb, type:'scatter', mode:'lines', name:'|G(jω)| (dB)'
    }], {
      title: 'Magnitude',
    ...
    Plotly.newPlot('bodePh', [{
      x: f, y: phDeg, type:'scatter', mode:'lines', name:'∠G(jω) (deg)'
    }], {
      title: 'Phase',
      xaxis:{ type:'log', title:'Frequency (Hz)' },
...
// =========================
// Laplace Tool (practical helper)
// =========================
function laplaceToolHtml(){
  return `
    <div class="tool-title">Laplace Helper</div>
    <div class="tool-sub">Common pairs + 2nd-order parameters (ωn, ζ) + step-response estimates.</div>

    <div class="tool-help">
      <b>Common Laplace pairs:</b><br/>
      1 ↔ 1/s<br/>
      t ↔ 1/s²<br/>
      e^{-a t} ↔ 1/(s+a)<br/>
      sin(ωt) ↔ ω/(s²+ω²)<br/>
      cos(ωt) ↔ s/(s²+ω²)<br/><br/>
      <b>2nd order canonical:</b><br/>
      G(s)=ωn²/(s²+2ζωn s+ωn²)
    </div>

    <div class="tool-grid" style="margin-top:10px;">
      <div class="tool-field">
        <label>Natural frequency ωn (rad/s)</label>
        <input id="lapWn" type="number" value="1000" step="10"/>
      </div>
      <div class="tool-field">
        <label>Damping ratio ζ</label>
        <input id="lapZeta" type="number" value="0.7" step="0.05"/>
      </div>
    </div>

    <div class="tool-actions">
      <button class="btn" id="lapCalcBtn">Compute</button>
      <button class="btn ghost" id="lapResetBtn">Reset</button>
    </div>

    <div id="lapOut" class="tool-help" style="margin-top:12px;"></div>
  `;
}

function laplaceCompute(){
  const wn = Number(document.getElementById('lapWn')?.value);
  const z  = Number(document.getElementById('lapZeta')?.value);
  if(!(wn > 0)) throw new Error("ωn must be > 0.");
  if(!(z >= 0)) throw new Error("ζ must be >= 0.");

  const wd = wn * Math.sqrt(Math.max(0, 1 - z*z));

  // Standard estimates
  const Mp = (z < 1) ? Math.exp(-z*Math.PI/Math.sqrt(1 - z*z)) : 0;
  const MpPct = Mp * 100;

  const Ts2 = (z > 0) ? 4/(z*wn) : Infinity;           // 2% settling
  const Tp  = (z < 1 && wd > 0) ? Math.PI/wd : Infinity;

  const poles = (z < 1)
    ? `s = -ζωn ± jωd = ${(-z*wn).toFixed(2)} ± j${wd.toFixed(2)}`
    : `real poles (over/critical damping). dominant pole ~ -ζωn = ${(-z*wn).toFixed(2)}`;

  const form = `G(s) = ωn² / (s² + 2ζωn s + ωn²)
= ${(wn*wn).toFixed(2)} / (s² + ${(2*z*wn).toFixed(2)} s + ${(wn*wn).toFixed(2)})`;

  return { wn, z, wd, MpPct, Ts2, Tp, poles, form };
}

function laplaceRender(){
  try{
    const r = laplaceCompute();
    const out = document.getElementById('lapOut');
    if(!out) return;

    out.innerHTML = `
      <b>Canonical form:</b><br/>${r.form.replaceAll('\n','<br/>')}<br/><br/>
      <b>Poles:</b><br/>${r.poles}<br/><br/>
      <b>Step estimates:</b><br/>
      Overshoot M<sub>p</sub> ≈ ${isFinite(r.MpPct) ? r.MpPct.toFixed(2) : '∞'}%<br/>
      Settling (2%) T<sub>s</sub> ≈ ${isFinite(r.Ts2) ? r.Ts2.toFixed(6) : '∞'} s<br/>
      Peak time T<sub>p</sub> ≈ ${isFinite(r.Tp) ? r.Tp.toFixed(6) : '∞'} s
    `;
  }catch(e){
...
// Euler / Complex Tool
// =========================
function eulerToolHtml(){
  return `
    <div class="tool-title">Euler / Complex Helper</div>
    <div class="tool-sub">Rectangular ↔ polar and e^{jθ} = cosθ + j sinθ. Useful for dq / space vectors (角度).</div>

    <div class="tool-grid">
      <div class="tool-field">
        <label>Rectangular: a</label>
        <input id="euA" type="number" value="1" step="0.01"/>
      </div>
      <div class="tool-field">
        <label>Rectangular: b (for a + j b)</label>
        <input id="euB" type="number" value="0" step="0.01"/>
      </div>

      <div class="tool-field">
        <label>Polar: magnitude r</label>
        <input id="euR" type="number" value="1" step="0.01"/>
      </div>
      <div class="tool-field">
        <label>Polar: angle θ (deg)</label>
        <input id="euDeg" type="number" value="0" step="1"/>
      </div>

      <div class="tool-field">
        <label>Euler: θ (deg) for e^{jθ}</label>
        <input id="euEulerDeg" type="number" value="30" step="1"/>
      </div>
      <div class="tool-field">
        <label>Result</label>
    ...
    <div class="tool-actions">
      <button class="btn" id="euRectToPolarBtn">Rect → Polar</button>
      <button class="btn" id="euPolarToRectBtn">Polar → Rect</button>
      <button class="btn" id="euEulerBtn">Compute e^{jθ}</button>
      <button class="btn ghost" id="euResetBtn">Reset</button>
    </div>

    <div class="tool-help">
      Rect → Polar: r = √(a²+b²), θ = atan2(b,a).<br/>
      Polar → Rect: a = r cosθ, b = r sinθ.
    </div>
  `;
}

function eulerRectToPolar(){
  const a = Number(document.getElementById('euA')?.value);
  const b = Number(document.getElementById('euB')?.value);
  const r = Math.hypot(a,b);
  const deg = Math.atan2(b,a) * 180/Math.PI;

  const R = document.getElementById('euR');
  const D = document.getElementById('euDeg');
  const O = document.getElementById('euOut');
  if(R) R.value = r.toFixed(6);
  if(D) D.value = deg.toFixed(6);
  if(O) O.value = `${r.toFixed(6)} ∠ ${deg.toFixed(6)} deg`;
}

function eulerPolarToRect(){
  const r = Number(document.getElementById('euR')?.value);
  const deg = Number(document.getElementById('euDeg')?.value);
  const th = deg * Math.PI/180;

  const a = r * Math.cos(th);
  const b = r * Math.sin(th);

  const A = document.getElementById('euA');
  const B = document.getElementById('euB');
  const O = document.getElementById('euOut');
  if(A) A.value = a.toFixed(6);
  if(B) B.value = b.toFixed(6);
  if(O) O.value = `${a.toFixed(6)} + j ${b.toFixed(6)}`;
}

function eulerComputeEj(){
  const deg = Number(document.getElementById('euEulerDeg')?.value);
  const th  = deg * Math.PI/180;
  ...
  const O = document.getElementById('euOut');
  if(O) O.value = `e^{j${deg}°} = ${a.toFixed(6)} + j ${b.toFixed(6)}`;
}

function eulerReset(){
  const set=(id,v)=>{ const el=document.getElementById(id); if(el) el.value=v; };
  set('euA','1'); set('euB','0');
  set('euR','1'); set('euDeg','0');
  set('euEulerDeg','30');
  set('euOut','');
}
...
function show(viewId){
  ['homeView','articleView','aboutView','tagsView','toolsView'].forEach(id=>{
